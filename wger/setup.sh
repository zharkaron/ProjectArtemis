...
docker compose exec -T wger-web python3 manage.py shell -c "
from django.contrib.auth.models import User
if not User.objects.filter(username='zharkaron').exists():
    User.objects.create_superuser('zharkaron', 'zharkaron@zharkaron.lab', '^O\$kSr&2*d2YLi')
    print('Superuser zharkaron created successfully')
else:
    print('Superuser zharkaron already exists')
"
